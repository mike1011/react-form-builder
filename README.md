...
- npm install mike1011/react-form-builder
- in `package.json` - add the below lines

```
"postinstall": "cd ./node_modules/react-form-builder2 && npm install && npm run build:dist"
```

- add `jquery` too as a dependency - `npm install jquery --save-dev`

- `dist` folder is also committed, to be on safe side.
